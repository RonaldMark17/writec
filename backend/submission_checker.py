...
def check_submission(job, submission, text, checkpoint, save_checkpoint):
    ...
    if mode == 'copyleaks' and provider is None:
        ...
        webhook = os.getenv('COPYLEAKS_WEBHOOK_BASE_URL', '').rstrip('/')
        if not webhook.startswith('https://') or 'localhost' in webhook:
            raise ValueError('Configure COPYLEAKS_WEBHOOK_BASE_URL as the public HTTPS backend URL.')
        ...
        while time.monotonic() < deadline:
            ...
            if rows[0].get('provider_error'):
                raise ValueError('The similarity provider failed. Review provider configuration before retrying.')
    ...
    result.update({'title': 'Similarity check', 'scanStatus': 'Completed',
        ...
        'summary': 'Copyleaks internet check and classroom comparison.' if provider else
